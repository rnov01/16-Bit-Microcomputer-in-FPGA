...
def main():
    ...
    for i in range(len(asm_whitespace_removed)):
        ...
        if asm_whitespace_removed[i].find('(') != -1:
            ...
            label_index += 1
            #symbol_extracted_asm[i] = symbol_extracted_asm[i+1]
    ...
    for i in range(len(symbol_extracted_asm)):
        ...
        if symbol_extracted_asm[i].startswith('@'):
            ...
            #print(symbol_extracted_asm[i])
            #print(dest, ' ', comp, ' ', jump)
            #print(current_instruction._destc.index(dest), ' ', current_instruction._compc.get(comp))
            try: 
